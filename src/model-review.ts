...
export const GO_MODULES_MODEL_PROMPT = `You are reviewing Go module metadata for reproducibility, upgrade safety, and ownership.
...
export const GO_MODULES_MODEL_SCHEMA: Record<string, unknown> = {
  type: "object",
...
export interface ModelModulesObservation {
  id: string;
  title: string;
  category: string;
...
export interface ModelModulesReview {
  assessment: {
  ...
  observations: ModelModulesObservation[];
}
...
export async function applyModelModulesReview(
  ctx: RuleContext,
  output: ModelModulesReview,
  evidenceById: Map<string, PreparedEvidenceItem>,
...
export async function runModelModulesReview(
  ctx: RuleContext,
  analysis: Analysis,
  files: DiscoveryFile[],
  staticSeverities: StaticSeverity[] = [],
  ...
  try {
    const result = await ctx.model.review<ModelModulesReview>(request);
...
function severityRank(severity: StaticSeverity | ModelModulesObservation["severity"]): number {
  switch (severity) {
    case "critical":
      return 4;
    case "high":
...
function maxSeverity(values: Array<StaticSeverity | ModelModulesObservation["severity"]>): StaticSeverity {
