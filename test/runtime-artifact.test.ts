...
test("the published runtime executes without node_modules", async () => {
  const artifact = await mkdtemp(join(tmpdir(), "go-modules-artifact-"));
  const repository = await mkdtemp(join(tmpdir(), "go-modules-target-"));
  ...
  assert.equal(envelope.result.adversary.name, "go-modules");
